...
const manifest = JSON.parse(fs.readFileSync('.next/server/pages-manifest.json', 'utf-8'));
...
(async () => {
  ...
  fs.appendFileSync(
    DEBUG,
    'Total size:' +
      toFormattedBytes(sortedFiles.map(([, size]) => size).reduce((curr, prev) => curr + prev, 0)) +
      '\n\n',
  );
